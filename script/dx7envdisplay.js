function makeEnvelope(r1Ele, r2Ele, r3Ele, r4Ele, l1Ele, l2Ele, l3Ele, l4Ele, canvas) {
	...
	canvas.width =
		parseInt(canvasParentStyle.width, 10) -
		parseInt(canvasParentStyle.paddingLeft, 10) -
		parseInt(canvasParentStyle.paddingRight, 10);
	canvas.height =
		parseInt(canvasParentStyle.height, 10) -
		parseInt(canvasParentStyle.paddingBottom, 10) -
		parseInt(canvasParentStyle.paddingBottom, 10);
	...
	let update = function() {
        ...
        */
		...
		let scaleText =
			'x' +
			((stepOneX + stepTwoX + stepThreeX + stepFourX) / canvasWidth).toLocaleString(undefined, { maximumFractionDigits: 3 });
		ctx.fillStyle = '#42cbc7';
	...
	for (let prop in envInputs) {
		envInputs[prop].addEventListener('input', function(event) {
			update();
		});
	}
